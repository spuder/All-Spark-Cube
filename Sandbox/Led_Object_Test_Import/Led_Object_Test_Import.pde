...
void parseFile(String[] loadedFile)
{
  ...
  for (int i = 0; i < loadedFile.length; i++)
  {
      ...
      if( pieces.length == 4) // if you get to the end of a line while parsing...
      {
        ...
        String keyString = pieces[0];               println("Created hashmap key pieces[i]- :" + keyString);
...
void mousePressed()
{
   ...
   Led aTemporaryDrawLed = hashMapOfLeds.get("led000");
   println("led000 - " + aTemporaryDrawLed.getLedLocaitonY() +" ");  
